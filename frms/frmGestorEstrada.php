...
//Inserir registo
        ...
        if (filter_has_var(INPUT_POST, 'tGuardar')) {
            ...
            $gestrada = new GestorEstrada(0, $id_gestor,$data,$id_estrada,$nomeado,$substituto,$interino,$datai,$dataf,$just,$ass,$ativo);
            $dal->Inserir($gestrada);
            $gestrada = new GestorEstrada();
             echo "<script type=\"text/javascript\"> alert('Os registos foram enviados com sucesso!');</script>";
        }        
//Alterar registo
        if (filter_has_var(INPUT_POST, 'btalterar')) {
            $id_gestrada = filter_input(INPUT_POST, 'tId_gestrada');
            $id_gestor = filter_input(INPUT_POST, 'tId_gestor');
            $data = filter_input(INPUT_POST, 'tData');
            ...
            $cx = new Conexao();
            $dal = new DALGestorEstrada($cx);
            $gestrada = new GestorEstrada($id_gestrada,$id_gestor,$data,$id_empreitada,$nomeado,$substituto,$interino,$datai,$dataf,$just,$ass,$ativo);
            $dal->Alterar($gestrada);
            $gestrada = new GestorEstrada();
        }
//Excluir registo
        if (filter_input(INPUT_GET, 'op') == 'excluir') {
            $conexao = new Conexao();
            $dal = new DALGestorEstrada($conexao);
...
//Alterar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dal = new DALGestorEstrada($conexao);
            $gestrada = $dal->CarregaGestorEstrada(filter_input(INPUT_GET, 'cod'));
        }
        ?> 
    </head>  
     <?php include_once '../estrutura/header.php';?>
    <body>
         <h1 class="op">REGISTAR - Gestor de Estrada</h1>
        ...
        <form  style=" height:420px;width: 500px;"method="post" name='gestrada' id="gestrada" action="../estrutura/gestor_main.php?pg=gestrada&op=listar">           
            ...
            <select style="height: 25px;width: 143px;text-align: left;" name ="tId_gestor" id="cId_gestor" >
             <?php
                        $valor ="";
                        $conexao = new Conexao();
                        $dal = new DALGestorEstrada($conexao);
                        ...
                        ?> 
                <option selected=""><?php echo $gestrada->getId_gestor(); ?></option></select> 
             
            <p><label style="width: 70px;margin-left: 0px" class="label_1" for="cId_estrada" >Estrada</label>
            <select style="height: 25px;width: 143px;text-align: center;" name ="tId_estrada" id="cId_estrada" >         
                     ...
                     <?php
                        ...
                        While ($reg = $res->fetch_array()) {
                            echo "<option value=\"" . $reg['id_estrada'] . "\">" .$reg['ilha']." - ". $reg['id_estrada']." - ".$reg['codigo'] ." - ". $reg['toponimo'] ."</option>";
                        }
                        ?>
                <option selected=""><?php echo $gestrada->getId_estrada(); ?></option></select>                
            ...
            <p><label  for="cNomeado">Nomeado</label>
                    <select class="alt" name ="tNomeado" id="cNomeado">
                        <option>0</option>
                        <option>1</option>
                        <option selected=""><?php echo $gestrada->getNomeado(); ?></option></select>                
                <label style="margin-left: 20px;" for="cSubstituto">Substituto</label>
                    <select class="alt" name ="tSubstituto" id="cSubstituto">
                        <option>0</option>
                        <option>1</option>
                        <option selected=""><?php echo $gestrada->getSubstituto(); ?></option></select>                  
                <label style="margin-left: 20px;" for="cInterino">Interino</label>
                    <select class="alt" name ="tInterino" id="cInterino">
                        <option>0</option>
                        <option>1</option>
                        <option selected=""><?php echo $gestrada->getInterino(); ?></option></select>
            </fieldset>     
             <p><label style="width: 80px;margin-left: 0px"class="label_1" for="cDatai" >Data início</label>
                <input style="height:25px;margin-left: 0px;" type="date" value= "<?php echo $gestrada->getDatai(); ?>" name ="tDatai" id="cDatai" class="align" required="true" />                
             <p><label style="width: 80px;margin-left: 0px"class="label_1" for="cDataf" >Data fim</label>
                <input style="height:25px;margin-left: 0px;" type="date" value= "<?php echo $gestrada->getDataf(); ?>" name ="tDataf" id="cDataf" class="align" required="true" />                   
             <p><label style="width: 80px;margin-left: 0px"class="label_1"for="cJust">Justificação</label>
                 <textarea style="height:50px;margin-left: 0px;width: 350px;text-align: left;"required type="text" name ="tJust" id="cJust" ><?php echo utf8_encode($gestrada->getJust()); ?></textarea>               
             <p><label for="cAtivo">Ativo</label>
                <select style="height: 28px;width: 40px;margin-left: 50px;"  name ="tAtivo" id="cAtivo">
                    <option>0</option>
                    <option>1</option>
                    <option selected=""><?php echo $gestrada->getAtivo(); ?></option></select>     
                ...
                <!--------------------------------------------------------------------------------------------> 
                  <div style=" height: 25px; padding: 5px; width: 590px; margin: 0 auto;"id="botoes">
                   ...
                   if ($gestrada->getId_gestrada() == 0) {
                    ...
                    <input type="text" hidden=""  name="tId_gestrada" id="cId_gestrada" value="<?php echo $gestrada->getId_gestrada(); ?>"/>          
                <?php } ?>
                     </div> 
        </form><br>
<!--------------------------------------------------- TABELA ---------------------------------------------->
<a style="text-align: center;display: block;font-family: calibri;font-size: 12pt;text-decoration: none;color:brown;" target='_blank' href = '../estrutura/gestor_main.php?pg=tabgestrada'>| CONSULTAR POR ILHA |</a><br>
   <table id="est" accept-charset="UTF-8" width ="100%" border='1' class="tabela" style="border-color: #848484;display: block;margin: 0 auto;">
            ...
            <tr> 
                <td class="title" align="center" >ID Gestor de Estrada</td>
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'gestrada');
            }
            $cx = new Conexao();
            $dal = new DALGestorEstrada($cx);
            ...
            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'gestor_main.php?pg=gestrada&op=excluir&cod=' . $registo['id_gestrada'];
                $linkalterar = 'gestor_main.php?pg=gestrada&op=alterar&cod=' . $registo['id_gestrada'];
                ?>  
                <tr> 
                    ...
                    <td  class="tab"width="12%"><?php echo $registo["just"]; ?></td>
        ...
        <a id="voltar"style="text-align:center;" href="../estrutura/gestor_main.php?pg=gestrada">|INÍCIO|</a>
        <footer>
