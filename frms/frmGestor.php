...
//Inserir registo
        ...
        if (filter_has_var(INPUT_POST, 'tGuardar')) {
            ...
            $cx = new Conexao();
            $dalgestor = new DALGestor($cx);
            $gestor = new Gestor(0, $nome_gestor, $nasc, $grauac, $nif, $endr, $corr, $tel1, $tel2,$data_reg);
            $dalgestor->Inserir($gestor);
            $gestor = new Gestor();
...
//Alterar registo
    ...
    <body>
        ...
        <form style=" height:450px;width: 500px;"method="post" name='gestor' id="gestor" action="../estrutura/admin_main.php?pg=regisgestor&op=listar">           
            <p><label for="cData_reg"style="margin-left:64px; ;">Data</label>
            <input type="date"name="tData_reg"id="cData_reg"value= "<?php echo $gestor->getData_reg(); ?>" required="" style="margin-left: 24px;height: 30px;width: 250px;"><p> 
            
          <label  style="margin-left: 53px;" for="cNome_gestor">Nome</label>
                   <select style="height: 33px;color: grey;width: 255px;margin-left: 30px;" value="<?php echo utf8_encode($gestor->getNome_gestor()); ?>" name ="tNome_gestor" id="cNome_gestor" >
                       
                        <?php
                        $valor = "";
                        $conexao = new Conexao();
                        $dal = new DALUsuario($conexao);
                        $res = $dal->cmbUsuario($valor);
                        While ($reg = $res->fetch_array()) {
                            echo "<option value=\"" . $reg['nome'] . "\">" . $reg['nome'] . "</option>";
                        }
                        ?>
                       <option selected=""><?php echo $gestor->getNome_gestor(); ?></option></select>
                                          
            <p><label for="cNasc">Data Nascimento</label>
            ...
            <input type="text" name="tTel2"id="cTel2"value="<?php echo $gestor->getTel2(); ?>"style="margin-left: 20px;height: 30px;width: 250px;"/><br><br>
                ...
                if ($gestor->getId_Gestor() == 0) {
                    ?>
             <input style="margin-left:200px;width: 100px;"type="submit"class="cmd" id="cGuardar"value="Guardar"name ="tGuardar"/>
                     
                    <?php
                } else {
                    ?>
                 <input style="margin-left: 140px;width: 100px;"class="cmd" type="submit" name="btalterar" id="btalterar" value="Alterar"/>
...
<!--------------------------------------------------- TABELA ---------------------------------------------->
           ...
           <?php
            ...
            <a id="voltar"style="text-align:center;" href="admin_main.php?pg=regisgestor">|INÍCIO|</a>
            <footer>
