...
<html>
    ...
    <head>
        ...
        <style>form#cMarc{height: 410px;}select#cMarc{height: 25px;}</style>
...
//Inserir registo
        ...
        if (filter_has_var(INPUT_POST, 'tGuardar')) {
            ...
            $marcad = new Marcadores(0,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $marc, $sentido, $ass);
            $cx = new Conexao();
 ...
 //Alterar registo
            ...
            $sentido = filter_input(INPUT_POST, 'tSentido');
            $ass = filter_input(INPUT_POST, 'tAss');
            $marcad = new Marcadores($id_marc,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $marc,$sentido, $ass);
            $cx = new Conexao();
            $dal = new DALMarcadores($cx);
            $dal->Alterar($marcad);
            $marcad = new Marcadores();
...
//Alterar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            ...
            $marcad = $marcad->CarregaMarcadores(filter_input(INPUT_GET, 'cod'));
        }   
            ?>
    </head>
    <body>
        <h1 class="op">REGISTAR - Marcadores</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1><br>
            <form method="post" onsubmit="return pk()" id="cMarc" action="../estrutura/levantamento_main.php?pg=marcadores&op=listar">
                ...
                <input value="<?php echo $marcad->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"placeholder="0"/>
                ...
                <p><label class="label_1"for="cSentido">Sentido</label>
                    ...
                    if ($marcad->getId_marc() == 0) {
                       ...
                       <input style="margin-left:225px;" type="submit"class="cmd" id="cGuardar"value="Guardar"name ="tGuardar"/>
                        <input type="button"class="cmd" id="cVer"value="Registos anteriores"name ="tVer"onclick ="location.href = '../estrutura/levantamento_main.php?pg=tabmarcadores'"/>
                        ...
                        <input hidden="" type="text"  name="tId_marc" id="id_marc" value="<?php echo $marcad->getId_marc(); ?>"/>          
                    <?php } ?>
            ...
            <table id="est" accept-charset="UTF-8" width ="110%" border="1" class="tabela">
                ...
                $resultado = $marcad->Localizar($valor);

                While ($registo = $resultado->fetch_array()) {
                    $linkexcluir = 'levantamento_main.php?pg=marcadores&op=excluir&cod=' . $registo['id_marc'];
                    $linkalterar = 'levantamento_main.php?pg=marcadores&op=alterar&cod=' . $registo['id_marc'];
                    ?>  
                    <tr>                      
