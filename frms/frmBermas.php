...
//Inserir registo
            ...
            $bermas = new Bermas(0,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf,$larg, $sentido, $ass);
            $cx = new Conexao();
            $dalbermas = new DALBermas($cx);
            $dalbermas->Inserir($bermas);
            $bermas = new Bermas();
...
//Alterar registo
            ...
            $sentido = filter_input(INPUT_POST, 'tSentido');         
            $ass = filter_input(INPUT_POST, 'tAss');
            $bermas = new Bermas($id_berma,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf,$larg, $sentido,  $ass);
            $cx = new Conexao();
            $dalbermas = new DALBermas($cx);
            $dalbermas->Alterar($bermas);
            $bermas = new Bermas();
            }
            //Excluir registo
            ...
            if (filter_input(INPUT_GET, 'op') == 'alterar') {
                $conexao = new Conexao();
                $dalbermas = new DALBermas($conexao);
                $bermas = $dalbermas->CarregaBermas(filter_input(INPUT_GET, 'cod'));
            }
        ?> 
    </head>
       <?php include_once '../estrutura/header.php'; ?> 
    <body>
        <h1 class="op">REGISTAR Bermas</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1><br>
            <form method="post" onsubmit="return pk()" id="bermas" action="../estrutura/levantamento_main.php?pg=bermas&op=listar">        
                ...
                <p><label class="label_1" for="cAlterar">Alteração</label>
                    ...
                    <input value="<?php echo $bermas->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"placeholder="0"/>
                ...
                <p><label class="label_1"for="cSentido">Sentido</label>
                    <select class="sentido" name ="tSentido" id="cSentido">					
                        ...
                        <option selected><?php echo $bermas->getSentido(); ?></option></select><br><br>
                  ...
                  <!--------------------------------------------------------------------------------------------> 
                    ...
                    if ($bermas->getId_berma() == 0) {
                        ...
                        <input type="button"class="cmd" id="cVer"value="Registos anteriores"name ="tVer"onclick ="location.href = '../estrutura/levantamento_main.php?pg=tabbermas'"/>
                        ...
                        <input hidden="" type="text"  name="tId_berma" id="id_berma" value="<?php echo $bermas->getId_berma(); ?>"/>          
                    <?php } ?>
            ...
            <table id="est" accept-charset="UTF-8" width ="120%" border="1" class="tabela">
                ...
                $cx = new Conexao();
                $dalbermas = new DALBermas($cx);
                $resultado = $dalbermas->Localizar($valor);

                While ($registo = $resultado->fetch_array()) {
                    $linkexcluir = 'levantamento_main.php?pg=bermas&op=excluir&cod=' . $registo['id_berma'];
                    $linkalterar = 'levantamento_main.php?pg=bermas&op=alterar&cod=' . $registo['id_berma'];
                    ?>  
                    <tr>                      
