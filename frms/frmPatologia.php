...
<html>
    ...
    <head>
        ...
        if (filter_has_var(INPUT_POST, 'tGuardar')) {
            ...
            $patolog = new Patologia(0,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf,$grupo,$id_talude,$banq, $via, $brm, $sobra, $pass, $sentido,  $id_item, $foto1, $foto2, $ass);
            $cx = new Conexao();
...
//Alterar registo   
        if (filter_has_var(INPUT_POST, 'btalterar')) {
            ...
            $patolog = new Patologia($id_patolog, $alt, $reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $grupo,$id_talude,$banq, $via, $brm, $sobra, $pass, $sentido, $id_item, $foto1, $foto2, $ass);          
            $dalpatolog->Alterar($patolog);
            $patolog = new Patologia();
        }
//Excluir registo
...
//Alterar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dalpatolog = new DALPatologia($conexao);
            $patolog = $dalpatolog->CarregaPatologias(filter_input(INPUT_GET, 'cod'));
        }
        ?>
    </head>
     <?php include_once '../estrutura/header.php';?>   
    <body>
        <h1 class="op">REGISTAR Patologia</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1><br>
        <form style="width:900px;height: 830px;" method="post" onsubmit="return pk()" name="patolog" id="patolog" action="../estrutura/levantamento_main.php?pg=patologia&op=listar">
                ...
                <p><label class="label_1" for="cAlterar">Alteração</label>
                    <select value="<?php echo $patolog->getAlt(); ?>"class="alt" name ="tAlterar" id="cAlterar">
                        <option>0</option>
                        <option>1</option>
                        <option selected><?php echo $patolog->getAlt(); ?></option></select>
                       &nbsp;<label  for="cReg">Registo</label>
                <input value="<?php echo $patolog->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"placeholder="0"/>   
               ...
               function sobra(){
                 ...
                 <select style="width:50px;height:25px;" value="<?php echo $patolog->getSobra(); ?>" name ="tSobra" id="cSobra"onclick="sobra()">
                    <option value=""></option>    
                    <option value="1">1</option>
                    <option selected><?php echo $patolog->getSobra(); ?></option></select>
                 ...
                 <select style="width:50px;height:25px;" value="<?php echo $patolog->getPass(); ?>" name ="tPass" id="cPass">
                    <option value=""></option>    
                    <option value="1">1</option>
                    <option selected><?php echo $patolog->getPass(); ?></option></select>
               ...
               <div style=" height: 25px; padding: 5px; width: 900px; margin: 0 auto;">
                ...
                if ($patolog->getId_patolog() == 0) {
                    ...
                    <input type="button"class="cmd" id="cVer"value="Registos anteriores"name ="tVer"onclick ="location.href ='../estrutura/levantamento_main.php?pg=tabpatologias'"/>
                    ...
                    <input type="text" hidden="" name="tId_patolog" id="id_patolog" value="<?php echo $patolog->getId_patolog(); ?>"/>          
                <?php } ?>
               </div>
        ...
        <table id="est" accept-charset="UTF-8" width ="140%" border="1" class="tabela">
            ...
            $resultado = $dalpatologia->Localizar($valor);

            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'levantamento_main.php?pg=patologia&op=excluir&cod=' . $registo['id_patolog'];
                $linkalterar = 'levantamento_main.php?pg=patologia&op=alterar&cod=' . $registo['id_patolog'];
                ?>  
                <tr>                      
