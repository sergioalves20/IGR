...
<html>
    <head>
        ...
        <style>
               form#trecho{width:750px;height: 450px;}
        </style>           
        <?php
...
//Inserir registo        
        if (filter_has_var(INPUT_POST, 'tGuardar')) {
            ...
            $trecho = new Trecho(0,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki,$sitioi, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf,$sitiof, $lat_cf, $lat_nf, $long_cf, $long_nf, $ass);
            $cx = new Conexao();
...
//Alterar registo
        if (filter_has_var(INPUT_POST, 'btalterar')) {
            ...
            $trecho = new Trecho($id_trecho,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki,$sitioi, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf,$sitiof, $lat_cf, $lat_nf, $long_cf, $long_nf, $ass);          
            $dal->Alterar($trecho);
            $trecho = new Trecho();
        }
//Excluir registo
...
//Alterar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dal = new DALTrecho($conexao);
            $trecho = $dal->CarregaTrecho(filter_input(INPUT_GET, 'cod'));
        }
            ?>
    </head>
    <?php include_once '../estrutura/header.php';?>
    <body>
        <h1 class="op">REGISTAR - Trecho de estrada</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1><br>
            <form method="post" onsubmit="return pk()"name="trecho" id="trecho"action="../estrutura/levantamento_main.php?pg=trecho&op=listar">
                ...
                <input value="<?php echo $trecho->getReg(); ?>"type="text" name ="tRegisto"  id="cRegisto" class="align" size= "2"placeholder="0"/>		
            ...
            <div style=" height: 25px; padding: 5px; width: 900px; margin: 0 auto;"id="botoes">                  
             <?php            
                if ($trecho->getId_trecho() == 0) {
                    ...
                    <input type="button"class="cmd" id="cVer"value="Registos anteriores"name ="tVer"onclick ="location.href = '../estrutura/levantamento_main.php?pg=tabtrecho'"/>
        ...
        <br>
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'tId_trecho');
            }
            $cx = new Conexao();
            $dal = new DALTrecho($cx);
            ...
            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'levantamento_main.php?pg=trecho&op=excluir&cod=' . $registo['id_trecho'];
                $linkalterar = 'levantamento_main.php?pg=trecho&op=alterar&cod=' . $registo['id_trecho'];
                ?>  
                <tr>                      
