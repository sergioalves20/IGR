...
<html>
    ...
    <body>       
            <h1>BASE DA FAIXA DE RODAGEM - Registos válidos anteriores a&nbsp;<?php echo date('Y-m-d'); ?></h1>
       <form class="tab" name='tab'method="post">
            <input type="text" name="fxrodbase" id='fxrodfund'placeholder="ID Estrada" style="width:120px" />
             &nbsp;&nbsp;<input id="listar" style="background-color: #CD950C;width: 80px;" class="cmd" type="submit" name='localizar' value="Selecionar"/><br><br>
        </form>
            <p><a id="voltar"style="text-align:center;" href="../frms/frmFxrodBase.php">|VOLTAR|</a>
        <table style="margin: 0 auto;" accept-charset="UTF-8" width ="80%" border="1" class="tabela">
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'fxrodbase');
            }
            $cx = new Conexao();
            $dal = new DALFxrodBase($cx);
            $resultado = $dal->TabFxrodBase($valor);
