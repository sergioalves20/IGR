...
<html>
 ...
 //Retificar registo
            if (filter_has_var(INPUT_POST, 'btalterar')) {
                ...
                $ctrafego = new Ctrafego($id_ctrafego,$alt,$reg, $id_estrada, $id_trecho, $data, $hora, $maquina, $id_posto, $altsolo, $disteixo, $sentido, $horai, $horaf, $datai, $dataf, $ndias, $vmedia, $lig, $pes, $tmda, $ass,$arq,$data_arq);
                $cx = new Conexao();
                $dal = new DALCtrafego($cx);
                $dal->Rectificar($ctrafego);
                echo "<script type=\"text/javascript\"> alert('Os registos foram alterados!');</script>";
            $ctrafego = new CTrafego();   
        }
//Excluir registo
...
//Retificar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dal = new DALCtrafego($conexao);
            $ctrafego = $dal->CarregaCTrafegoRect(filter_input(INPUT_GET, 'cod'));
        }
            ?>
    </head> 
    <?php include_once '../estrutura/header.php';?>
    <body>
        <h1 class="op">ALTERAR - Contagem de tráfego</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1>
         <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=alterar">|VOLTAR|</a>
              <form method="post" name="cont" onsubmit="return pk()" id="cont" action="../estrutura/ALTERAR_main.php?pg=ctrafego&op=listar"> 
                <p><label class="label_1" for="cId_sing">ID Contagem</label>
                    <input style="background-color:#ccffcc;border-style: none;height: 25px;color:#7F7F7F;" type="text" use readonly="true"  name="tId_ctrafego" id="id_sing" value="<?php echo $ctrafego->getId_ctrafego(); ?>"class="align" size="6"/>            
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" type="text" value= '<?php echo $ctrafego->getData(); ?>' name ="tData" id="cData" class="align" size= "8" Use readonly="true" />
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" value= '<?php echo $ctrafego->getHora(); ?>' name ="tHora" id="cHora" class="align" size= "6" Use readonly="true" />
           <hr> 	               
                <p><label class="" for="cAlterar">Alteração</label>
                    <select class="alt" name ="tAlterar" id="cAlterar">
                        <option  selected>0</option>
                        <option>1</option><?php echo $ctrafego->getAlt(); ?></select>                 
                    <label  for="cReg">Registo</label>
                    <input value="<?php echo $ctrafego->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"required=""/>	
                <label class="" for="cArq">Arquivar</label>
                <select   class="alt" name ="tArq" id="cArq">
                    <option  selected="">0</option>
                    <option>1</option><?php echo $ctrafego->getArq(); ?></select>          
                &nbsp;<label  for="cData_arq">Data</label>
                <input style="height: 20px"value="<?php echo $ctrafego->getData_arq(); ?>" type="date" name ="tData_arq"  id="cData_arq" class="align" size= "6"/>
                ...
                <p><label class="label_1"for="cDatai">Data Início</label>
                    <input type="text" value= '<?php echo $ctrafego->getDatai(); ?>' name ="tDatai" id="cDatai" class="align" size= "7"/>
                    <label style="margin-left:30px;" for="cHorai">Hora Início</label>	
                    <input type="text" value= '<?php echo $ctrafego->getHorai(); ?>' name ="tHorai" id="cHorai" class="align" size= "7"/>
                <p><label class="label_1"for="cDataf">Data Fim</label>
                    <input type="text" value= '<?php echo $ctrafego->getDataf(); ?>' name ="tDataf" id="cDataf" class="align" size= "7"/>
                    <label style="margin-left:38px;"for="cHoraf">Hora Fim</label>	
                    <input type="text" value= '<?php echo $ctrafego->getHoraf(); ?>' name ="tHoraf" id="cHoraf" class="align" size= "7"/>
        ...
        <input type="text" name="ctrafego" id='ctrafego' placeholder="ID Estrada"style="width: 120px;" />
        &nbsp;&nbsp;<input id="listar" style="background-color: #CD950C;width: 80px;" class="cmd" type="submit" name='localizar' value="Selecionar"/><br><br>
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'ctrafego');
            }
            $cx = new Conexao();
            $dal = new DALCtrafego($cx);
            ...
            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'alterar_main.php?pg=ctrafego&op=excluir&cod=' . $registo['id_ctrafego'];
                $linkalterar = 'alterar_main.php?pg=ctrafego&op=alterar&cod=' . $registo['id_ctrafego'];
                ?>  
                <tr>                      
           ...
           <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=ctrafego">|INÍCIO|</a>
        <footer>
