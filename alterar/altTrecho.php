...
//Retificar registo
        if (filter_has_var(INPUT_POST, 'btalterar')) {
            ...
            $reg = filter_input(INPUT_POST, 'tRegisto');
            ...
            $sitioi = trim(filter_input(INPUT_POST, 'tSitioi'));
            ...
            $sitiof = trim(filter_input(INPUT_POST, 'tSitiof'));
            ...
            $conexao = new Conexao();
            $dal = new DALTrecho($conexao);
            $trecho = new Trecho($id_trecho,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki,$sitioi, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf,$sitiof, $lat_cf, $lat_nf, $long_cf, $long_nf, $ass, $arq, $data_arq);          
            $dal->Rectificar($trecho);
            $trecho = new Trecho();
        }
//Excluir registo
...
//Retificar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dal = new DALTrecho($conexao);
            $trecho = $dal->CarregaTrechoRect(filter_input(INPUT_GET, 'cod'));
        }
            ?>
    </head>
    <?php include_once '../estrutura/header.php';?>
    <body>
        <h1 class="op">ALTERAR - Trecho de estrada</h1>
        <h1 class="op"><a href="#est" style="text-decoration: none;color:blue;">| TABELA |</a></h1>
         ...
         <form style="width:750px;height: 470px;" method="post" onsubmit="return pk()"name="trecho" id="trecho"action="../estrutura/alterar_main.php?pg=trecho&op=listar">
                <p><label class="label_1" for="cId_trecho">ID Trecho</label>
                <input style="background-color:#ccffcc;border-style: none;height: 25px;color:#7F7F7F;" type="text" use readonly="true"  name="tId_trecho" id="id_trecho" value="<?php echo $trecho->getId_trecho(); ?>"class="align" size="6"/>            
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" type="text" value= '<?php echo $trecho->getData(); ?>' name ="tData" id="cData" class="align" size= "8" Use readonly="true" />
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" value= '<?php echo $trecho->getHora(); ?>' name ="tHora" id="cHora" class="align" size= "6" Use readonly="true" />
           <hr> 
                <p><label class="label_1" for="cAlterar">Alteração</label>
                    <select class="alt" name ="tAlterar" id="cAlterar">
                        <option selected>0</option>
                        <option>1</option><?php echo $trecho->getAlt(); ?></select>
                  <label  for="cRegisto">Registo</label>
                  <input value="<?php echo $trecho->getReg(); ?>"type="text" name ="tRegisto"  id="cRegisto" class="align" size= "2"required=""/>		
             <label class="" for="cArq">Arquivar</label>
                <select   class="alt" name ="tArq" id="cArq">
                    <option  selected="">0</option>
                    <option>1</option><?php echo $trecho->getArq(); ?></select>          
                &nbsp;<label  for="cData_arq">Data</label>
                <input style="height: 20px"value="<?php echo $trecho->getData_arq(); ?>" type="date" name ="tData_arq"  id="cData_arq" class="align" size= "6"/>
            <p><label class="label_1" for="cId_estrada">ID Estrada</label>
                <input value="<?php echo $trecho->getId_estrada(); ?>" autofocus required="" type="text" name ="tId_estrada"  id="cId_estrada" class="align" size= "5"/>
                <input type="text" hidden="" name="tId_trecho" id="cId_trecho" value="<?php echo $trecho->getId_trecho(); ?>"/> 
                <!-------------------------------------------- PK INÍCIO ----------------------------------------------------------------->
            <p><label style="color:brown;" class="label_1"for="cPkinicio">pk Início</label>
                <input  value='<?php echo $trecho->getPkinicio(); ?>' required="" type="text" name ="tPkinicio" id="cPkinicio" class="align" size= "5"placeholder="0.000"/>
                <label style="margin-left:15px;" for="cAltitd_pki">Altitude(m)</label>
                &nbsp;<input style="margin-left:5px;" value='<?php echo $trecho->getAltitd_pki(); ?>' type="text" name ="tAltitd_pki" id="cAltitd_pki" class="align" size= "5" placeholder="0"/>
             
            <p><label style="color:brown;"class="label_1"for="cSitio">Sítio</label>
                <input  value='<?php echo $trecho->getSitioi(); ?>'type="text" name ="tSitioi" id="cSitioi" style="height:18px;" size= "58"/>           
                <!-------------------------------------------- LATITUDE ------------------------------------------------------------------->
            <p><label class="label_1"for="cLatitudeGi">Latitude</label>
                ...
                <input  value='<?php echo $trecho->getLat_ci(); ?>'charset= "utf8" type="text" required="" name ="tLat_ci" id="cLatitudei" class="align" size= "8" placeholder="00º00'00''"Use readonly="true"/>
                <label for="cLatitudeN"></label>					
                <input  value='<?php echo $trecho->getLat_ni(); ?>'type="text"  required=""name ="tLat_ni" id="cLatitudeNi" class="align" size= "10"placeholder="00.00000000" Use readonly="true"/>                          
            ...
            <p><label class="label_1" for="cLongitudeGi">Longitude</label> 
                ...
                <input  value='<?php echo $trecho->getLong_ci(); ?>'type="text"  required="" name ="tLong_ci" id="cLongitudei" class="align" size= "8" placeholder="00°00'00''"Use readonly="true">
                <label for="cLongitudeN"></label>
                <input  value='<?php echo $trecho->getLong_ni(); ?>'type="text" required="" name ="tLong_ni" id="cLongitudeNi" class="align" size= "10" placeholder="00.00000000" Use readonly="true"/>	     
            ...
            <p><label style="color:brown;"class="label_1"for="cPkfim">pk Fim</label>
                <input  value='<?php echo $trecho->getPkfim(); ?>'type="text" name ="tPkfim" id="cPkfim" class="align" size= "5"placeholder="0.000"/>
                <label style="margin-left:15px;" for="cAltitd_pkf">Altitude(m)</label>
                &nbsp;<input style="margin-left:5px;" value='<?php echo $trecho->getPkfim(); ?>' type="text" name ="tAltitd_pkf" id="cAltitd_pkf" class="align" size= "5" placeholder="0"/><br><p>	
                
             <p><label style="color:brown;"class="label_1"for="cSitiof">Sítio</label>
            <input  value='<?php echo $trecho->getSitiof(); ?>'type="text" name ="tSitiof" id="cSitiof" style="height:18px;" size= "58"/>
            ...
            <p><label class="label_1"for="cLatitudeGf">Latitude</label>
                ...
                <input  value='<?php echo $trecho->getLat_cf(); ?>'charset= "utf8" type="text" name ="tLat_cf" id="cLatitudef" class="align" size= "8" placeholder="00º00'00''"Use readonly="true"/>
                <label for="cLatitudeN"></label>					
                <input  value='<?php echo $trecho->getLat_nf(); ?>'type="text" name ="tLat_nf" id="cLatitudeNf" class="align" size= "10"placeholder="00,00000000" Use readonly="true"/>                          
            ...
            <p><label class="label_1" for="cLongitudeGf">Longitude</label> 
                ...
                <input  value='<?php echo $trecho->getLong_cf(); ?>'type="text" name ="tLong_cf" id="cLongitudef" class="align" size= "8" placeholder="00°00'00''"Use readonly="true">
                <label for="cLongitudeNf"></label>
                <input  value='<?php echo $trecho->getLong_nf(); ?>'type="text" name ="tLong_nf" id="cLongitudeNf" class="align" size= "10" placeholder="00.00000000" Use readonly="true">
        ...
        <input type="text" name="trecho" id='trecho' placeholder="ID Estrada" style= "width: 120px;"/>
        &nbsp;&nbsp;<input id="listar" style="background-color: #CD950C;width:80px;" class="cmd" type="submit" name='localizar' value="Selecionar"/><br><br>
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'trecho');
            }
            $cx = new Conexao();
            ...
            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'alterar_main.php?pg=trecho&op=excluir&cod=' . $registo['id_trecho'];
                $linkalterar = 'alterar_main.php?pg=trecho&op=alterar&cod=' . $registo['id_trecho'];
                ?>  
                <tr>                      
           ...
           <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=trecho">|INÍCIO|</a>
        <footer>
