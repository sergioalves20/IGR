...
//Retificar registo
            if (filter_has_var(INPUT_POST, 'btalterar')) {
                ...
                $fxrod = new Fxrod($id_fxrod, $alt, $reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $nvias, $larg1, $larg2, $larg3, $larg4, $larg5, $larg6, $ass, $arq, $data_arq);
                $cx = new Conexao();
                $dal = new DALFxrod($cx);
                $dal->Rectificar($fxrod);
                $fxrod = new Fxrod();
            }
...
//Retificar registo
            if (filter_input(INPUT_GET, 'op') == 'alterar') {
                $conexao = new Conexao();
                $dalfxrod = new DALFxrod($conexao);
                $fxrod = $dalfxrod->CarregaFxrodRect(filter_input(INPUT_GET, 'cod'));
            }
            ?> 
        </head>
        <?php include_once '../estrutura/header.php'; ?> 
        <body>
            <h1 class="op">ALTERAR - Faixa de rodagem</h1>
            ...
            <form method="post" onsubmit="return pk()" id="fxrod" action="../estrutura/alterar_main.php?pg=fxrod&op=listar">
                <p><label class="label_1" for="cId_fxrod">ID Fxrod</label>
                <input style="background-color:#ccffcc;border-style: none;height: 25px;color:#7F7F7F;"type="text" use readonly="true"  name="tId_fxrod" id="id_fxrod" value="<?php echo $fxrod->getId_fxrod(); ?>"class="align" size="6"/>            
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" value= '<?php echo $fxrod->getData(); ?>' name ="tData" id="cData" class="align" size= "8" Use readonly="true" />
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" value= '<?php echo $fxrod->getHora(); ?>' name ="tHora" id="cHora" class="align" size= "6" Use readonly="true" />
           <hr> 
                <p><label class="label_1" for="cAlterar">Alteração</label>
                    ...
                    <input value="<?php echo $fxrod->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"/>
                 <label class="" for="cArq">Arquivar</label>
                <select   class="alt" name ="tArq" id="cArq">
                    <option  selected="">0</option>
                    <option>1</option><?php echo $fxrod->getArq(); ?></select>            
                &nbsp;<label  for="cData_arq">Data</label>
                <input style="height: 20px"value="<?php echo $fxrod->getData_arq(); ?>" type="date" name ="tData_arq"  id="cData_arq" class="align" size= "6"required="true"/>
            ...
            <table id="est" accept-charset="UTF-8" width ="130%" border="1" class="tabela">
                ...
                $resultado = $dalfxrod->LocalizarRect($valor);

                While ($registo = $resultado->fetch_array()) {
                    $linkexcluir = 'alterar_main.php?pg=fxrod&op=excluir&cod=' . $registo['id_fxrod'];
                    $linkalterar = 'alterar_main.php?pg=fxrod&op=alterar&cod=' . $registo['id_fxrod'];
                    ?>  
                    <tr>                      
           ...
           <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=fxrod">|INÍCIO|</a>
