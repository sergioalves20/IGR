...
//Retificar registo
        if (filter_has_var(INPUT_POST, 'btalterar')) {
            ...
            $talude = new Talude($id_talude,$alt,$reg, $id_estrada, $data, $hora, $pkinicio, $altitd_pki, $lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $nat, $altura, $inclin,$tipo, $sentido, $nbanq, $ass,$arq,$data_arq);          
            $daltalude->Rectificar($talude);
            $talude = new Talude();
        }
//Excluir registo
...
//Retificar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $daltalude = new DALTalude($conexao);
            $talude = $daltalude->CarregaTaludeRect(filter_input(INPUT_GET, 'cod'));
        }
        ?>
    </head>
    <?php include_once '../estrutura/header.php';?>
    <body>
        <h1 class="op">ALTERAR - Talude</h1>
        ...
        <form style="height:520px;"  method="post" name="talude" id="talude" onsubmit="return pk()" action="../estrutura/alterar_main.php?pg=talude&op=listar" >
             <p><label class="label_1" for="cId_talude">ID Talude</label>
                <input style="background-color:#ccffcc;border-style: none;height: 25px;color:#7F7F7F;"type="text" use readonly="true"  name="tId_talude" id="cId_talude" value="<?php echo $talude->getId_talude(); ?>"class="align" size="6"/>            
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" type="text" value= '<?php echo $talude->getData(); ?>' name ="tData" id="cData" class="align" size= "8" Use readonly="true" />
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;"type="text" value= '<?php echo $talude->getHora(); ?>' name ="tHora" id="cHora" class="align" size= "6" Use readonly="true" />
           <hr>             
             <p><label class="label_1" for="cAlterar">Alteração</label>
                <select   class="alt" name ="tAlterar" id="cAlterar">
                    <option>0</option>
                    <option>1</option>
                    <option selected=""><?php echo $talude->getAlt(); ?></option></select> 
                 &nbsp;<label  for="cReg">Registo</label>
                 <input value="<?php echo $talude->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "3"required="true"/>
             <label class="" for="cArq">Arquivar</label>
                <select   class="alt" name ="tArq" id="cArq">
                    <option>0</option>
                    <option>1</option>
                    <option selected=""><?php echo $talude->getArq(); ?></option></select>            
                &nbsp;<label  for="cData_arq">Data</label>
                <input style="height: 20px"value="<?php echo $talude->getData_arq(); ?>" type="date" name ="tData_arq"  id="cData_arq" class="align" size= "6"/>   
        ...
        <input type="text" name="talude" id='talude' placeholder="ID Estrada" style= "width: 120px;" />
        ...
        <table id="est" accept-charset="UTF-8" width ="130%" border="1" class="tabela">
            ...
            $resultado = $daltalude->LocalizarRect($valor);

            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'alterar_main.php?pg=talude&op=excluir&cod=' . $registo['id_talude'];
                $linkalterar = 'alterar_main.php?pg=talude&op=alterar&cod=' . $registo['id_talude'];
                ?>  
                <tr>                      
          ...
          <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=talude">|INÍCIO|</a>
        <footer>
