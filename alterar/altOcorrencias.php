...
//Retificar registo
         if (filter_has_var(INPUT_POST, 'btalterar')){
            ...
            $ocorr = new Ocorrencias($id_ocor,$alt,$reg, $data, $hora,$id_estrada,$pkinicio,$altitd_pki,$lat_ci, $lat_ni, $long_ci, $long_ni, $pkfim, $altitd_pkf, $lat_cf, $lat_nf, $long_cf, $long_nf, $sentido, $ocor,$foto1, $foto2, $ass,$arq,$data_arq);
            $cx = new Conexao();
            $dal = new DALOcorrencias($cx);
            $dal->Rectificar($ocorr);
            $ocorr = new Ocorrencias();
         }
//Excluir registo
...
//Retificar registo
        if (filter_input(INPUT_GET, 'op') == 'alterar') {
            $conexao = new Conexao();
            $dal = new DALOcorrencias($conexao);
            $ocorr = $dal->CarregaOcorrenciasRect(filter_input(INPUT_GET, 'cod'));
        }
        ?>       
    </head>
    ...
    <body>
        ...
        <form method="post" name="ocor" onsubmit="return pk()" id="ocor" action="../estrutura/alterar_main.php?pg=ocorrencia&op=listar">             
            <p><label class="label_1" for="cId_ocor">ID Ocorrência</label>
                <input style="background-color:#ccffcc;border-style: none;height: 25px;color:#7F7F7F;" type="text" use readonly="true"  name="tId_ocor" id="id_ocor" value="<?php echo $ocorr->getId_ocor(); ?>"class="align" size="6"/>            
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" type="text" value= '<?php echo $ocorr->getData(); ?>' name ="tData" id="cData" class="align" size= "8" Use readonly="true" />
                <input style="margin-left:0px;background-color:#ccffcc;border-style: none;height: 25px;" value= '<?php echo $ocorr->getHora(); ?>' name ="tHora" id="cHora" class="align" size= "6" Use readonly="true" />
            <hr>         
            <p><label class="" for="cAlt">Alteração</label>
                    <select style="margin-left:35px;" class="alt"  name ="tAlterar" id="cAlterar">
                        <option selected>0</option>
                        <option>1</option><?php echo $ocorr->getAlt(); ?></select>               
                <label  for="cReg">Registo</label>
                <input value="<?php echo $ocorr->getReg(); ?>"type="text" name ="tReg"  id="cReg" class="align" size= "2"required=""/>
                <label class="" for="cArq">Arquivar</label>
                <select   class="alt" name ="tArq" id="cArq">
                    <option  selected="">0</option>
                    <option>1</option><?php echo $ocorr->getArq(); ?></select>          
                &nbsp;<label  for="cData_arq">Data</label>
                <input style="height: 20px"value="<?php echo $ocorr->getData_arq(); ?>" type="date" name ="tData_arq"  id="cData_arq" class="align" size= "6"/>            
            <p><label class="label_1" for="cId_estrada">ID Estrada</label>
                <input value="<?php echo $ocorr->getId_estrada(); ?>" autofocus required="" type="text" name ="tId_estrada"  id="cId_estrada" class="align" size= "5"/>
              <!-------------------------------------------- PK INÍCIO ----------------------------------------------------------------->
            <p><label style="color:brown;" class="label_1"for="cPkinicio">pk Início</label>
                <input  value='<?php echo $ocorr->getPkinicio(); ?>' required="" type="text" name ="tPkinicio" id="cPkinicio" class="align" size= "5"placeholder="0.000"/>
                <label style="margin-left:15px;" for="cAltitd_pki">Altitude(m)</label>
                &nbsp;<input style="margin-left:5px;" value='<?php echo $ocorr->getAltitd_pki(); ?>' type="text" name ="tAltitd_pki" id="cAltitd_pki" class="align" size= "5" placeholder="0"/>
                <!-------------------------------------------- LATITUDE ------------------------------------------------------------------->
            <p><label class="label_1"for="cLatitudeGi">Latitude</label>
                ...
                <input  value='<?php echo $ocorr->getLat_ci(); ?>'charset= "utf8" type="text" required="" name ="tLat_ci" id="cLatitudei" class="align" size= "8" placeholder="00º00'00''"Use readonly="true"/>
                <label for="cLatitudeN"></label>					
                <input  value='<?php echo $ocorr->getLat_ni(); ?>'type="text"  required=""name ="tLat_ni" id="cLatitudeNi" class="align" size= "10"placeholder="00.00000000" Use readonly="true"/>                          
            ...
            <p><label class="label_1" for="cLongitudeGi">Longitude</label> 
                ...
                <input  value='<?php echo $ocorr->getLong_ci(); ?>'type="text"  required="" name ="tLong_ci" id="cLongitudei" class="align" size= "8" placeholder="00°00'00''"Use readonly="true">
                <label for="cLongitudeN"></label>
                <input  value='<?php echo $ocorr->getLong_ni(); ?>'type="text" required="" name ="tLong_ni" id="cLongitudeNi" class="align" size= "10" placeholder="00.00000000" Use readonly="true"/>	     
            ...
            <p><label style="color:brown;"class="label_1"for="cPkfim">pk Fim</label>
                <input  value='<?php echo $ocorr->getPkfim(); ?>'type="text" name ="tPkfim" id="cPkfim" class="align" size= "5"placeholder="0.000"/>
                <label style="margin-left:15px;" for="cAltitd_pkf">Altitude(m)</label>
                &nbsp;<input style="margin-left:5px;" value='<?php echo $ocorr->getPkfim(); ?>' type="text" name ="tAltitd_pkf" id="cAltitd_pkf" class="align" size= "5" placeholder="0"/><br><p>	
                <!------------------------------------------- LATITUDE ------------------------------------------------------------------------------>
            <p><label class="label_1"for="cLatitudeGf">Latitude</label>
                ...
                <input  value='<?php echo $ocorr->getLat_cf(); ?>'charset= "utf8" type="text" name ="tLat_cf" id="cLatitudef" class="align" size= "8" placeholder="00º00'00''"Use readonly="true"/>
                <label for="cLatitudeN"></label>					
                <input  value='<?php echo $ocorr->getLat_nf(); ?>'type="text" name ="tLat_nf" id="cLatitudeNf" class="align" size= "10"placeholder="00,00000000" Use readonly="true"/>                          
            ...
            <p><label class="label_1" for="cLongitudeGf">Longitude</label> 
                ...
                <input  value='<?php echo $ocorr->getLong_cf(); ?>'type="text" name ="tLong_cf" id="cLongitudef" class="align" size= "8" placeholder="00°00'00''"Use readonly="true">
                <label for="cLongitudeNf"></label>
                <input  value='<?php echo $ocorr->getLong_nf(); ?>'type="text" name ="tLong_nf" id="cLongitudeNf" class="align" size= "10" placeholder="00.00000000" Use readonly="true"/>	     
                ...
                <!--------------------------------------------------------------------------------------------------------------------------------->
                 <p><label class="label_1" for="cSentido">Sentido</label>
                    ...
                    <option selected><?php echo $ocorr->getSentido()?></option></select>        
            <p><label class="label_1"for="cOcor">Ocorrência:</label><br>	
                <textarea  name="tOcor" id="cOcor"><?php echo $ocorr->getOcor(); ?></textarea>                    
            <p><label class="label_1"for="cFoto1"id="foto1" >Foto 1</label>
                <input value='<?php echo $ocorr->getFoto1(); ?>'type="file"name="tFoto1"id="cFoto1" class="align"/>
            <p><label class="label_1"for="cFoto2"id="foto2" >Foto 2</label>
                <input value='<?php echo $ocorr->getFoto2(); ?>'type="file"name="tFoto2"id="cFoto2" class="align"/><br><br>
              <!-----------------------------------------------------------------------------------------------------> 
                <div style=" height: 25px; padding: 5px; width: 900px; margin: 0 auto;"id="botoes">                  
             <?php         
                if ($ocorr->getId_ocor() == 0) {
                    ?>
                    <?php
                } else {
                    ?>  
        ...
        <input type="text" name="ocor" id='ocor' placeholder="ID Estrada" style= "width: 120px;"/>
        &nbsp;&nbsp;<input id="listar" style="background-color: #CD950C;width: 80px;" class="cmd" type="submit" name='localizar' value="Selecionar"/>
            ...
            if (filter_input(INPUT_POST, 'localizar')) {
                $valor = filter_input(INPUT_POST, 'ocor');
            }
            $cx = new Conexao();
            $dal = new DALOcorrencias($cx);
            $resultado = $dal->LocalizarRect($valor);
            While ($registo = $resultado->fetch_array()) {
                $linkexcluir = 'alterar_main.php?pg=ocorrencia&op=excluir&cod=' . $registo['id_ocor'];
                $linkalterar = 'alterar_main.php?pg=ocorrencia&op=alterar&cod=' . $registo['id_ocor'];
                ?>  
                <tr>                      
           ...
           <a id="voltar"style="text-align:center;" href="alterar_main.php?pg=ocorrencia">|INÍCIO|</a>
           <footer>
